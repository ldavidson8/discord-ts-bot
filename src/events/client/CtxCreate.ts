import { ActionRowBuilder, GuildChannel, GuildMember, PermissionFlagsBits } from 'discord.js';
...
import { checkCooldown } from '../../functions/cooldown.js';
...
export default class CtxCreate extends Event {
	...
	public async run(ctx: Context): Promise<any> {
		...
		if (ctx.isInteraction) {
			command = this.client.commands.get(ctx.interaction?.commandName as string);
		} else {
			command =
		...
		const isDev: boolean =
			ctx.author && this.client.config.owners?.includes(ctx.author.id) ? true : false;
		if (
			!(
				ctx.inGuild &&
				...
				.send({
					embeds: [
						this.client
							...
							.desc(
								`❌ I am missing ${missingBotPermissions
									.map((perm) => `\`${perm}\``)
		...
		if (this.client.development === true && ctx.guild?.id !== this.client.config.guildId) {
			await ctx.errorReply('I am unavailable right now due to development mode.');
			return;
		}
		...
		if (command.permissions) {
			...
			if (command.permissions?.user) {
				if (!(isDev || (ctx.member as GuildMember).permissions.has(command.permissions.user))) {
			...
			if (command.permissions?.dev && this.client.config.owners) {
				if (!isDev) {
					let sentMsg = await ctx.errorReply('This command is restricted to my developer(s) only.');
		...
		if (!ctx.isInteraction) {
			if (ctx.args.includes('@everyone') || ctx.args.includes('@here')) {
				return await ctx.errorReply('You cannot mention everyone/here in your message.');
			}
		}
		...
		try {
			...
			return await ctx.errorReply('There was an error while executing this command.');
		} finally {
			...
			await this.client.webhooks.cmdlogs.send({
				...
				embeds: [
					this.client
						.embed()
						.desc(
							`✅ **${ctx.author.username}** ran the \`${command.name}\` ${slashOrPrefix} command in **${ctx.guild.name}** **(ID: \`${ctx.guild.id}\`)** within the **${ctx.channel}** channel.`,
						),
				],
			});
		}
